...
type S3 struct {
	...
	ConnectPort    int           `json:"connectPort"`    // 指定 TCP 连接端口，0 表示沿用 Endpoint 端口，不影响 HTTP Host / 签名
}
...
type WebDAV struct {
	...
	DNSRecordType  string        `json:"dnsRecordType"`  // 指定解析记录类型：A 或 CNAME
	DNSRecordValue string        `json:"dnsRecordValue"` // 指定解析记录值
	ConnectPort    int           `json:"connectPort"`    // 指定 TCP 连接端口，0 表示沿用 Endpoint 端口，不影响 HTTP Host
}
